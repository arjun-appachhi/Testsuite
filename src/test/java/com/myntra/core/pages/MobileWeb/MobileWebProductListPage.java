package com.myntra.core.pages.MobileWeb;

import com.myntra.core.pages.ProductDescPage;
import com.myntra.core.pages.ProductListPage;
import com.myntra.core.pages.ShoppingBagPage;
import com.myntra.core.pages.WishListPage;
import io.qameta.allure.Step;
import org.openqa.selenium.support.ui.ExpectedConditions;

import java.util.HashMap;

public class MobileWebProductListPage extends ProductListPage {

    @Step
    @Override
    public ProductDescPage selectFirstProductFromListPage() {
        utils.click(getLocator("lnkFirstProduct"), true);
        return ProductDescPage.createInstance();
    }

    @Step
    @Override
    public ProductListPage saveToWishlistFromListPage() {
        utils.waitForElementToBeVisible(getLocator("txaSellingPrice"));
        utils.wait(ExpectedConditions.elementToBeClickable(getLocator("btnSave")));
        utils.click(getLocator("btnSave"), true);
        return this;
    }

    @Step
    @Override
    public boolean isProductListDisplayed() {
        return utils.isElementPresent(getLocator("allProductList"), 10);
    }

    @Step
    @Override
    public boolean isProductSaved() {
        utils.wait(ExpectedConditions.elementToBeClickable(getLocator("btnSaved")));
        return (utils.isElementPresent(getLocator("btnSaved"), 10));
    }

    @Step
    private boolean isDiscountPriceAvailable() {
        utils.waitForElementToBeVisible(getLocator("txaDiscount"));
        return (utils.isElementPresent(getLocator("txaDiscount"), 10));
    }

    @Step
    @Override
    public HashMap<String, String> getProductDetails() {
        HashMap<String, String> productDetails = new HashMap<>();
        if (utils.isElementPresent(getLocator("txaSellingPrice"), 5)) {
            productDetails.put("Selling Price", utils.findElement(getLocator("txaSellingPrice"))
                                                     .getText()
                                                     .replace(",", "")
                                                     .trim());
        }
        isProductDiscountAvailable(productDetails);
        testExecutionContext.addTestState("productDetailsInPLP", productDetails);
        return productDetails;
    }

    @Step
    private void isProductDiscountAvailable(HashMap<String, String> productDetails) {
        if (utils.isElementPresent(getLocator("txaStrikedPrice"), 4)) {
            productDetails.put("Product Discount", utils.findElement(getLocator("txaDiscount"))
                                                        .getText());
            productDetails.put("Striked Price", utils.findElement(getLocator("txaStrikedPrice"))
                                                     .getText());
        } else {
            LOG.info("No discounted price is available");
        }
    }

    @Step
    @Override
    public WishListPage navigateToWishlist() {
        utils.click(getLocator("lnkWishlist"), true);
        return WishListPage.createInstance();
    }

    @Step
    @Override
    protected ProductListPage filterUsingDiscount() {
        utils.click(getLocator("btnFilter"), true);
        utils.click(getLocator("btnDiscountInFilter"), true);
        utils.click(getLocator("chkFilterOption"), true);
        utils.click(getLocator("btnApply"), true);
        utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        return this;
    }

    @Step
    @Override
    public ProductListPage navigateToPromotion() {
        utils.click(getLocator("btnFilter"), true);
        utils.click(getLocator("lblPromotions"), true);
        return this;
    }

    @Step
    @Override
    public ProductListPage selectFreeGiftUnderPromotions() {
        utils.click(getLocator("chkBoxPromotionsFreeGift"), true);
        utils.waitForElementToBeVisible(getLocator("btnApply"));
        utils.click(getLocator("btnApply"), true);
        utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        return this;
    }

    @Step
    @Override
    public boolean isPromotionAvailableInPLP() {
        return utils.isElementPresent(getLocator("btnFilter"), 4);
    }

    @Step
    @Override
    public ProductDescPage selectOneProductInConditionalDiscountProducts() {
        utils.click(getLocator("lnkFirstProduct"), true);
        return ProductDescPage.createInstance();
    }

    @Step
    @Override
    public ProductDescPage selectProductNotApplicableForConditionalDiscount() {
        utils.click(getLocator("lnkFirstProduct"), true);
        return ProductDescPage.createInstance();
    }

    @Step
    @Override
    public ProductListPage selectStaggeredComboUnderPromotions() {
        if (utils.isElementPresent(getLocator("chkBoxStaggeredCombo"), 2)) {
            utils.wait(ExpectedConditions.elementToBeClickable(getLocator("chkBoxStaggeredCombo")));
            utils.click(getLocator("chkBoxStaggeredCombo"), true);
            utils.waitForElementToBeVisible(getLocator("btnApply"));
            utils.click(getLocator("btnApply"), true);
            utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        }
        return this;
    }

    @Step
    @Override
    public ProductListPage selectBOGOUnderPromotions() {
        if (utils.isElementPresent(getLocator("chkBoxPromotionsBOGO"), 2)) {
            utils.wait(ExpectedConditions.elementToBeClickable(getLocator("chkBoxPromotionsBOGO")));
            utils.click(getLocator("chkBoxPromotionsBOGO"), true);
            utils.waitForElementToBeVisible(getLocator("btnApply"));
            utils.click(getLocator("btnApply"), true);
            utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        }
        return this;
    }

    @Step
    @Override
    public ProductListPage addToBagFromListPage() {
        utils.click(getLocator("lnkFirstProduct"), true);
        ProductDescPage.createInstance()
                       .selectSizeAndAddToBag();
        return this;
    }

    @Step
    @Override
    public ShoppingBagPage navigateToBag() {
        ProductDescPage.createInstance()
                       .goToBag();
        return ShoppingBagPage.createInstance();
    }

    @Step
    @Override
    public ProductListPage addSecondItemToBag() {
        utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        utils.click(getLocator("lnkSecondItem"), true);
        ProductDescPage.createInstance()
                       .selectSizeAndAddToBag();
        return this;
    }

    @Step
    @Override
    public ProductListPage addThirdItemToBag() {
        utils.isElementPresent(getLocator("btnDiscountInFilter"), 2);
        utils.click(getLocator("lnkThirdItem"), true);
        ProductDescPage.createInstance()
                       .selectSizeAndAddToBag();
        return this;
    }
}

